...
// for atspm-data-downloader - atspm_data_downloader.HostedServices/DownloaderServiceBase.cs
// 
...
/// <summary>
/// Abstract base class that consolidates common stream downloading logic for ATSPM datasets.
/// </summary>
public abstract class DownloaderServiceBase : HostedServiceBase
{
    ...
    /// <summary>
    /// Initializes a new instance of the <see cref="DownloaderServiceBase"/> class.
    /// </summary>
    protected DownloaderServiceBase(
        IServiceScopeFactory serviceProvider,
        HttpClient httpClient,
        IOptions<DownloaderConfiguration> options,
        IHostApplicationLifetime lifetime,
        ILogger logger,
        IDownloaderLogMessages log) : base(logger, serviceProvider)
    {
    ...
    /// <summary>
    /// Gets the base relative URL path (e.g., "api/v1/EventLog/StreamData" or "api/v1/Aggregation/StreamData").
    /// </summary>
    protected abstract string BaseRelativePath { get; }

    /// <summary>
    /// Gets the friendly dataset label for output file naming (e.g., "events" or "aggregations").
    /// </summary>
    protected abstract string DatasetLabel { get; }

    /// <summary>
    /// Core execution logic for executing stream dataset downloads.
    /// </summary>
    public override async Task Process(IServiceScope scope, Stopwatch? stopwatch = null, CancellationToken cancellationToken = default)
    {
        ...
        foreach (var locationIdentifier in _options.LocationIdentifiers)
        {
            try
            {
                ...
                {
                    relativeUrl = $"{BaseRelativePath.TrimEnd('/')}/{locationIdentifier}?start={startStr}&end={endStr}";
                }
                else
                {
                    relativeUrl = $"{BaseRelativePath.TrimEnd('/')}/{locationIdentifier}/{Uri.EscapeDataString(_options.DataType)}?start={startStr}&end={endStr}";
                }
                ...
                if (!response.IsSuccessStatusCode)
                {
                    ...
                    throw new HttpRequestException($"API returned error code {response.StatusCode} for location {locationIdentifier}.");
                }
